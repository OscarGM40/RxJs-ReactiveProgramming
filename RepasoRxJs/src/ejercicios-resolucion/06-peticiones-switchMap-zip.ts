...
(() => {
  ...
  /* EJERCICIO DOS SOLUCION DOS */
  /* fijate que zip recibe un arreglo de Observables,debo cumplir con ello */
  const obs2$ = getRequest(`${SW_API}/people/1`).pipe(
    switchMap(response => getRequest(response.starships[0])));
...
/*   zip(
    ...
    console.log('Personaje:', values[0]);
    console.log('Nave:', values[1]);
  }) */
